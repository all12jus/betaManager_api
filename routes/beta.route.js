var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __asyncValues = (this && this.__asyncValues) || function (o) {
    if (!Symbol.asyncIterator) throw new TypeError("Symbol.asyncIterator is not defined.");
    var m = o[Symbol.asyncIterator], i;
    return m ? m.call(o) : (o = typeof __values === "function" ? __values(o) : o[Symbol.iterator](), i = {}, verb("next"), verb("throw"), verb("return"), i[Symbol.asyncIterator] = function () { return this; }, i);
    function verb(n) { i[n] = o[n] && function (v) { return new Promise(function (resolve, reject) { v = o[n](v), settle(resolve, reject, v.done, v.value); }); }; }
    function settle(resolve, reject, d, v) { Promise.resolve(v).then(function(v) { resolve({ value: v, done: d }); }, reject); }
};
var _this = this;
(function () {
    var mongoose = require("mongoose");
    var express = require('express');
    var bodyParser = require('body-parser');
    var utils = require('./../utils');
    var Beta = require("../models/beta.model").betaModel;
    var Tester = require("../models/tester.model").testerModel;
    var router = express.Router();
    router.use([
        bodyParser.urlencoded({ extended: true }),
        bodyParser.json(),
        // bodyParser.text()
    ]);
    router.get("/", function (req, res) { return __awaiter(_this, void 0, void 0, function () {
        var outputObj, _a, _b, doc, c, o, e_1_1;
        var e_1, _c;
        return __generator(this, function (_d) {
            switch (_d.label) {
                case 0:
                    outputObj = [];
                    _d.label = 1;
                case 1:
                    _d.trys.push([1, 7, 8, 13]);
                    _a = __asyncValues(Beta.find());
                    _d.label = 2;
                case 2: return [4 /*yield*/, _a.next()];
                case 3:
                    if (!(_b = _d.sent(), !_b.done)) return [3 /*break*/, 6];
                    doc = _b.value;
                    // use `doc`
                    // var o = doc;
                    console.log(doc);
                    return [4 /*yield*/, Tester.find({
                            beta: mongoose.Types.ObjectId(doc.id)
                        })];
                case 4:
                    c = _d.sent();
                    o = {
                        beta: doc,
                        users: c
                    };
                    outputObj.push(o);
                    _d.label = 5;
                case 5: return [3 /*break*/, 2];
                case 6: return [3 /*break*/, 13];
                case 7:
                    e_1_1 = _d.sent();
                    e_1 = { error: e_1_1 };
                    return [3 /*break*/, 13];
                case 8:
                    _d.trys.push([8, , 11, 12]);
                    if (!(_b && !_b.done && (_c = _a["return"]))) return [3 /*break*/, 10];
                    return [4 /*yield*/, _c.call(_a)];
                case 9:
                    _d.sent();
                    _d.label = 10;
                case 10: return [3 /*break*/, 12];
                case 11:
                    if (e_1) throw e_1.error;
                    return [7 /*endfinally*/];
                case 12: return [7 /*endfinally*/];
                case 13: return [2 /*return*/, res.json(outputObj)];
            }
        });
    }); });
    router.post("/", function (req, res) {
        console.log(req.body);
        var result = utils.validateInputs(['name'], req.body);
        if (result.missingValues.length > 0 || result.extraneousValues.length > 0) {
            return res.status(400).json({ "error": "Missing Values", "missingValues": result.missingValues, "extraneousValues": result.extraneousValues });
        }
        var betaItem = new Beta(result.value);
        betaItem.save(function (err) {
            if (err != null) {
                return res.status(500).json({ error: "Database Error: " + err });
            }
            res.json({ err: (err != null) ? err : undefined, item: betaItem });
        });
    });
    router.get("/:beta_id/", function (req, res) { return __awaiter(_this, void 0, void 0, function () {
        var beta, c, o;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, Beta.findOne({
                        _id: req.params.beta_id
                    })["catch"](function (err) {
                        return res.status(400).json({ error: err });
                    })];
                case 1:
                    beta = _a.sent();
                    if (beta == null) {
                        return [2 /*return*/, res.status(400).json({ "error": "Invalid Parameter", "params": req.params.beta_id })];
                    }
                    return [4 /*yield*/, Tester.find({
                            beta: mongoose.Types.ObjectId(beta.id)
                        })["catch"](function (err) {
                            return res.status(400).json({ error: err });
                        })];
                case 2:
                    c = _a.sent();
                    o = {
                        beta: beta,
                        users: c
                    };
                    return [2 /*return*/, res.json(o)];
            }
        });
    }); });
    router.get("/:beta_id/tester", function (req, res) { return __awaiter(_this, void 0, void 0, function () {
        var beta, c;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, Beta.findOne({
                        _id: req.params.beta_id
                    })["catch"](function (err) {
                        return res.status(400).json({ error: err });
                    })];
                case 1:
                    beta = _a.sent();
                    if (beta == null) {
                        return [2 /*return*/, res.status(400).json({ "error": "Invalid Parameter", "params": req.params.beta_id })];
                    }
                    return [4 /*yield*/, Tester.find({
                            beta: mongoose.Types.ObjectId(beta.id)
                        })["catch"](function (err) {
                            return res.status(400).json({ error: err });
                        })];
                case 2:
                    c = _a.sent();
                    // o.users = "Hello";
                    // console.log(o);
                    // let o = {
                    //     // beta: beta,
                    //     users: c
                    // };
                    return [2 /*return*/, res.json(c)];
            }
        });
    }); });
    router.post("/:beta_id/tester", function (req, res) { return __awaiter(_this, void 0, void 0, function () {
        var beta, result, testerItem;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, Beta.findOne({
                        _id: req.params.beta_id
                    })["catch"](function (err) {
                        return res.status(400).json({ error: err });
                    })];
                case 1:
                    beta = _a.sent();
                    if (beta == null) {
                        return [2 /*return*/, res.status(400).json({ "error": "Invalid Parameter", "params": req.params.beta_id })];
                    }
                    result = utils.validateInputs(["last_name", "first_name", "email_address"], req.body);
                    if (result.missingValues.length > 0 || result.extraneousValues.length > 0) {
                        return [2 /*return*/, res.status(400).json({ "error": "Missing Values", "missingValues": result.missingValues, "extraneousValues": result.extraneousValues })];
                    }
                    testerItem = new Tester(result.value);
                    testerItem.beta = beta;
                    testerItem.save(function (err) {
                        if (err != null) {
                            return res.status(500).json({ error: "Database Error: " + err });
                        }
                        // res.json({err: err, item: testerItem});
                        res.json({ err: (err != null) ? err : undefined, item: testerItem });
                    });
                    return [2 /*return*/];
            }
        });
    }); });
    module.exports = router;
})();
